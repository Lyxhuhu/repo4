...
void Aproject1Character::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	// Bind fire event
	PlayerInputComponent->BindAction("Fire", IE_Pressed, this, &Aproject1Character::OnFire);
...
void Aproject1Character::OnFire()
{
	...
	if (ProjectileClass != nullptr)
	{
		...
		if (World != nullptr)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<Aproject1Projectile>(ProjectileClass, SpawnLocation, SpawnRotation);
			}
			else
			{
				...
				World->SpawnActor<Aproject1Projectile>(ProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
			}
		}
	}
...
//void Aproject1Character::TouchUpdate(const ETouchIndex::Type FingerIndex, const FVector Location)
//{
...
bool Aproject1Character::EnableTouchscreenMovement(class UInputComponent* PlayerInputComponent)
{
	if (FPlatformMisc::SupportsTouchInput() || GetDefault<UInputSettings>()->bUseMouseForTouch)
	{
		...
		//PlayerInputComponent->BindTouch(EInputEvent::IE_Repeat, this, &Aproject1Character::TouchUpdate);
		return true;
	}
